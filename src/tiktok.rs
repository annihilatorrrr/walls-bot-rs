...
pub static MATCH_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new("https://(?:[a-z]{2}.)(?P<host>?tiktok.com)/").unwrap());
...
pub async fn handler(
    ...
    if let Some(text) = scrub_urls(&message) && let Some(user) = message.from() &&
        ...
        let text = text.replace(&caps[HOST_MATCH_GROUP], "vxtiktok.com");
        let text = format!(
            "{}: {}",
...
mod test {
    ...
    #[test]
    fn verify_regex() {
        let items = vec!["https://www.tiktok.com/t/CuXFcC1LCza"];
        for item in items {
        ...
        assert!(!MATCH_REGEX.is_match("https://www.tiktok.com/@ludwig"));
        let caps = MATCH_REGEX
            .captures("https://www.tiktok.com/t/CuXFcC1LCza")
            .unwrap();
        assert_eq!(&caps[HOST_MATCH_GROUP], "www.tiktok.com");
        let caps = MATCH_REGEX
            .captures("https://tiktok.com/t/CuXFcC1LCza")
            .unwrap();
        assert_eq!(&caps[HOST_MATCH_GROUP], "tiktok.com");
    }
}
